...
function checkDistinctNames(selectors, actions) {
    ...
    if ((0, _lodash.includes)(allkeys, 'action')) throw new Error("reslice.checkDistinctNames: cannot use action as a selector or action name");
    if ((0, _lodash.includes)(allkeys, 'globalAction')) throw new Error("reslice.checkDistinctNames: cannot use globalAction as a selector or action name");
    if ((0, _lodash.includes)(allkeys, 'getRoot')) throw new Error("reslice.checkDistinctNames: cannot use getRoot as a selector or action name");
}
    ...
    if (reducer.$$combine) {
        ...
        reducer = {
            $$reducer: (0, _redux.combineReducers)((0, _lodash.mapValues)(reducer.$$reducers, function (r) {
                return buildReducer(r, store, myTag);
            })),
